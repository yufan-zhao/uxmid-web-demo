...
export default class Paging extends Component
{
     ...
     * @type {IFilter}
     */
    protected formItems: IPaginFilter = {};
            ...
            try
            {
                ...
                if(response)
                {
                    ...
                    if(this.pageIndex === 1)
                    {
                        ...
                        if (this.records[0])
                        {
                            this.records[0]._highlight = true;
                            this.$emit("on-row-click", this.records[0], 0);
                        }
                        else
                        {
                            this.$emit("no-data");
    ...
    private restoreRoute(): void
    {
        ...
        for(let name of Object.keys(parameters))
        {
            ...
            else
            {
                ...
                {
                    // 设置过滤条件的值
                    filter.value = ExtendUtils.decodeFilterValue(filter, parameters[name]);
     ...
     */
    private getSearchModel(): IPaginFilterModel
    {
        let seachModel = {
            data: {},
            params: {}
        ...
        Object.keys(this.filters).forEach(key =>
        {
            ...
            {
                seachModel.data[key] = value.id;
            }
            else
            {
                seachModel.data[key] = value;
            }
        });
