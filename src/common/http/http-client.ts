...
export default class HttpClient
{
    ...
    private getAxiosRequest(method: any, request: IHttpRequest): AxiosRequestConfig
    {
        ...
        let requestUrl = Type.isUndefined(request.urlPath) ?  request.url : StringUtils.formatString(request.url, request.urlPath);
    ...
    private async send(axiosRequest: AxiosRequestConfig): Promise<any>
    {
        return new Promise<any>((resolve, reject) =>
        {
            ...
            .catch((error: any) =>
            {
                ...
                let statusCode = error.response ? error.response.status : HTTP_RESPONSE_CODE.networkError;
    ...
    private async onRequestComplete(axiosResponse: AxiosResponse | any, axiosRequest: AxiosRequestConfig): Promise<any>
    {
        ...
        const entry = EnumUtils.getEntry(code, HTTP_RESPONSE_CODE);
        
        if(code === HTTP_RESPONSE_CODE.success || code === HTTP_RESPONSE_CODE.nodata)
        {
        ...
        else
        {
            ...
            if(code === HTTP_RESPONSE_CODE.failure || code === HTTP_RESPONSE_CODE.invalidParameter)
            {
    ...
    private resolveResponse(axiosResponse: AxiosResponse, axiosRequest: AxiosRequestConfig): IHttpResponse
    {
        ...
        const response: IHttpResponse =
        {
            code: resCode,
            request,
            content: resCode === HTTP_RESPONSE_CODE.success ? data : {data, msg: content.msg},
            extras,
            headers
        };
